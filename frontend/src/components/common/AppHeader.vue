...
<style scoped>
.site-header { position: sticky; z-index: 1000; top: 0; height: var(--header-height); border-bottom: 1px solid rgba(220,227,223,.9); background: rgba(255,255,255,.94); backdrop-filter: blur(14px); }
...
.main-nav a { display: flex; align-items: center; gap: 7px; padding: 10px 13px; border-radius: 6px; color: var(--color-muted); font-size: 14px; font-weight: 700; }
.main-nav a:hover, .main-nav a.router-link-active { color: var(--color-primary-dark); background: var(--color-primary-soft); }
.menu-button { display: none; border: 0; background: transparent; }
@media (max-width: 700px) {
  .header-inner { width: calc(100% - 28px); }
  .menu-button { display: grid; place-items: center; }
  .main-nav { position: absolute; top: var(--header-height); right: 0; left: 0; display: none; padding: 12px 14px 16px; border-bottom: 1px solid var(--color-line); background: #fff; box-shadow: var(--shadow-sm); }
