...
import { ArrowUpRight, Eye, Map, Route, Search, X } from 'lucide-vue-next'
...
function showSearchPage() {
  ...
  data.value = { items: searchItems.value.slice(start, start + state.size), total: searchItems.value.length }
}
async function load() {
  loading.value = true
  error.value = ''
  try {
    if (!searchQuery.value) {
      data.value = await getCourses(state)
      return
    }

    const first = await getCourses({ sort: state.sort, page: 1, size: 100 })
    const pageCount = Math.ceil(first.total / 100)
    const rest = pageCount > 1
      ? await Promise.all(Array.from({ length: pageCount - 1 }, (_, index) => getCourses({ sort: state.sort, page: index + 2, size: 100 })))
      : []
    const keyword = searchQuery.value.toLocaleLowerCase('ko-KR')
    searchItems.value = [first, ...rest]
      .flatMap((response) => response.items)
      .filter((course) => course.title.toLocaleLowerCase('ko-KR').includes(keyword))
    showSearchPage()
  } catch (err) { error.value = err.message }
  finally { loading.value = false }
}
function searchCourses() { searchQuery.value = searchInput.value.trim(); state.page = 1; load() }
function clearSearch() { searchInput.value = ''; searchQuery.value = ''; searchItems.value = []; state.page = 1; load() }
function setSort(value) { state.sort = value; state.page = 1; load() }
function setPage(value) { state.page = value; if (searchQuery.value) showSearchPage(); else load(); window.scrollTo({ top: 0, behavior: 'smooth' }) }
onMounted(load)
</script>
...
<template>
  <div class="page container">
    ...
    <form class="course-search panel" role="search" @submit.prevent="searchCourses">
      <Search :size="19" aria-hidden="true" />
      <input v-model="searchInput" type="search" maxlength="200" placeholder="코스 이름으로 검색" aria-label="코스 이름 검색" />
      <button v-if="searchQuery" class="search-clear" type="button" aria-label="검색 초기화" @click="clearSearch"><X :size="17" /></button>
    ...
    <StatePanel v-else-if="!data.items.length" :title="searchQuery ? `‘${searchQuery}’ 코스를 찾지 못했어요` : '아직 저장된 코스가 없어요'" :message="searchQuery ? '다른 코스 이름으로 검색해 보세요.' : '지도에서 spot을 장바구니에 담아 첫 코스를 만들어보세요.'" />
...
<style scoped>
.course-search { display: flex; align-items: center; gap: 10px; margin-bottom: 12px; padding: 10px 12px 10px 16px; }.course-search > svg { flex: 0 0 auto; color: var(--color-muted); }.course-search input { min-width: 0; flex: 1; height: 38px; border: 0; outline: 0; color: var(--color-text); background: transparent; font: inherit; font-size: 13px; }.course-search input::placeholder { color: var(--color-muted); }.course-search .button { min-height: 38px; padding-inline: 18px; }.search-clear { display: grid; width: 32px; height: 32px; flex: 0 0 auto; place-items: center; border: 0; border-radius: 50%; color: var(--color-muted); background: var(--color-surface, #f3f5f4); }.search-clear:hover { color: var(--color-text); }
...
@media (max-width: 900px) { .course-grid { grid-template-columns: repeat(2,1fr); } } @media (max-width: 600px) { .course-search { padding-left: 12px; }.course-search .button { padding-inline: 13px; }.course-grid { grid-template-columns: 1fr; } }
</style>
