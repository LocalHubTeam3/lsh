...
<template>
  <div class="page container">
    <header class="page-heading">
      <div><p class="eyebrow">LOCAL STORIES</p><h1>커뮤니티</h1><p>서울의 장소와 경험을 익명으로 편하게 나눠보세요.</p></div>
    ...
    <section class="board-tools panel">
      <form class="search-form" @submit.prevent="submitSearch">
        <Search :size="18" />
        <input v-model="draftSearch" type="search" maxlength="100" placeholder="제목이나 내용을 검색하세요" aria-label="게시글 검색어" />
    ...
    <section v-else class="post-list panel">
      <RouterLink v-for="post in data.items" :key="post.id" class="post-row" :to="`/community/${post.id}`">
        ...
        <div class="post-copy"><h2>{{ post.title }}</h2><p>{{ post.content }}</p><small v-if="post.location"><MapPin :size="12" />{{ post.location.title }}</small></div>
        <div class="post-meta"><span><UserRound :size="14" />익명</span><span>{{ formatDate(post.created_at) }}</span><span><Eye :size="14" />{{ post.views.toLocaleString('ko-KR') }}</span></div>
      </RouterLink>
    </section>
...
.post-copy { min-width: 0; }.post-copy h2 { overflow: hidden; margin: 0 0 8px; font-size: 17px; text-overflow: ellipsis; white-space: nowrap; }
.post-copy p { overflow: hidden; margin: 0; color: var(--color-muted); font-size: 13px; text-overflow: ellipsis; white-space: nowrap; }.post-copy small { display: flex; align-items: center; gap: 3px; margin-top: 7px; color: var(--color-primary); font-size: 10px; font-weight: 700; }
