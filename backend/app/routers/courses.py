...
from app.schemas.course import CourseCreate, CourseList, CourseOut, CourseSummary, CourseUpdate
...
def get_course(course_id: int, db: Session = Depends(get_db)) -> CourseOut:
    course = service.require_course(db, course_id); course.views += 1; db.commit(); db.refresh(course)
